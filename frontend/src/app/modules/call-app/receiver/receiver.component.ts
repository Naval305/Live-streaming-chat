...
export class ReceiverComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // this.remotePeerId = window.location.search.split('=')[2];
    this.initializePeer();
  }
  ...
  displayPeerId(id: string): void {
    this.remotePeerId = this.userData.peer_id;
    // Assuming SET_PEER is an action in your store
    // this.store.dispatch({ type: 'SET_PEER', payload: id });
  }
  ...
  initializeWebSocket(peerId: string): void {
    //  this.socket = new WebSocket(`${this.wsUrl}/ws/message/${this.remotePeerId}`);
